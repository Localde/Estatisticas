...
class Calculadora(object):
    def __init__(self, instancia):
        ...
        botões = (
        'Comb(n, k)', 'binomial(n, x, p)', 'poisson(l, x, t)', 'soma(n, p, maior, menor = 0)', 'media', 'desvio',
        'moda', 'mediana', 'variancia', 'p(x > k)', 'p(x >= k)', 'p(x < k)', 'p(x <= k)', 'p(k1 < x < k2)',
        ...
        for i in range(len(botões)):
            if i % 3 == 0:
                ...
                subframe.pack()
            a = Button(subframe, text=botões[i], bg='green', width=25, padx=5, command=partial(self.ColocarTexto, botões[i]))
            a.pack(side=LEFT)
